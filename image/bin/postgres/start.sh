...
function fill_conf_file() {
    ...
    sed -i "s/US\/Eastern/Asia\/Shanghai/g" /tmp/postgresql.conf
    sed -i "s/US\/Eastern/Asia\/Shanghai/g" /tmp/postgresql.conf
    sed -i "/log_min_duration_statement/s/0/500ms/g" /tmp/postgresql.conf
    echo 'checkpoint_timeout = 30min' >> /tmp/postgresql.conf
    echo 'max_wal_size = 5GB' >> /tmp/postgresql.conf
    echo 'checkpoint_completion_target = 0.9' >>/tmp/postgresql.conf
    echo 'wal_compression = on' >> /tmp/postgresql.conf
    sed -i "/log_rotation_size/s/0/200MB/g" /tmp/postgresql.conf
    echo 'log_min_error_statement = warning'>>/tmp/postgresql.conf
    sed -i "/shared_buffers/s/128MB/1GB/g" /tmp/postgresql.conf
    sed -i "/max_connections/s/100/500/g" /tmp/postgresql.conf
}
